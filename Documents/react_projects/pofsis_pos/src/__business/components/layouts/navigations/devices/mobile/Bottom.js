    ...
    const installHandler = async () => {
        if (install_state.prompt.data !== null) {
            ...
            if (outcome === 'accepted') {
                ...
                localStorage.removeItem('your_biz_session')
                window.location.href = `${domain('pm').url}/sso/rqx?from=${key('tsa_key')}&callback=${env()==='dev' || env()==='local'?'development':env()==='sb'?'sandbox':env()}&platform=business`
            }
        }
            ...
            {install_state.prompt.data !== null && (
                ...
                <Toolbar sx={{display:'flex', alignItems:'center', height:'68px', p:`0px ${xsm ?'8px':'16px'} !important`}}>
                        <Box height="32px" width="100%" display="flex" justifyContent="center" alignItems="center" color="#283745" gap="12px">
                            ...
                            <Box component={Button} onClick={installHandler} sx={{height:'36px', width:'103px', bgcolor:'#165320', color:'#FFFFFF', borderRadius:'6px', ':hover':{bgcolor:'#165320', color:'#FFFFFF'} }}>
            ...
            <Toolbar sx={{display:'flex', justifyContent: 'space-between', alignItems:'center', height:'54px', gap:(xsm ?'8px':'15px'), p:`0px ${xsm ?'8px':'16px'} !important`}}>
                    {
                        navigation_state.tabs.data.mobile.map((v,k) => (
                            <Box height="100%" width="56px" display="flex" alignItems="center" justifyContent="center" key={k} className={v.steps} onClick={() => handlerNavBtn(v.subdir)}>
                                <ListItemButton sx={{display:'flex', flexDirection: 'column', justifyContent:'center', px: '0px !important', color:Loc === v.ref ? '#11783C' : '#687B75', fontWeight:600}}>
                                    ...
                                    {(Loc[2] === 'tool-categories' && v.name === 'Tool Categories') ? 
                                        <Box className="marquee" width="50px">
                                            <Box className={Loc[2] === 'tool-categories' ? 'marqueestc' : 'marquees'} fontWeight={Loc[2] === v.ref ? 600 : 400} fontSize={11} color={ Loc[2] === v.ref ? "#11783C" : '#283745'}>{v.name} Example</Box>
                                        </Box>
                                    :
                                        <Box textAlign="center" fontWeight={Loc[2] === v.ref ? 600 : 400} fontSize={11} lineHeight="18px" color={ Loc[2] === v.ref ? "#11783C" : '#283745'} sx={{width:'100%', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis'}}>{v.name}</Box>
                                    }
                                </ListItemButton>
                            </Box>
                        ))
                    }
                    <Box height="100%" width="56px" display="flex" alignItems="center" justifyContent="center" onClick={()=> nav.push('/biz/more')}>
                        <ListItemButton sx={{display:'flex', flexDirection: 'column', justifyContent:'center', alignItems:'center', px:'0px !important' }}>
                            <IconButton disableFocusRipple={true} disableTouchRipple={true} sx={{padding:'4px !important', bgcolor:Loc[1] === 'more' ? '#F0F5F9':'#FFFFFF', borderRadius:'8px'}}>
