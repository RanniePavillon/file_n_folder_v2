...
export const List  = ({__SESSION, toolCategories, classes}) => {
    ...
    const handleClick = async(tid, v, category) =>{
        if (parseInt(process.env.REACT_APP_TID_PSL) !== tid) {
            if (v.owned) {
                ...
                (v.tools.length > 0 || (v.tools !== undefined && v.tools !== null ) ) && (
                    <Box key={k} display="flex" flexDirection="column" gap="12px">
                        ...
                        <Box display="grid" gridTemplateColumns="repeat(auto-fill,minmax(80px,1fr))" gap="8px">
                            {v.tools.map((t,i) => (
                                <Box component={Button} disableFocusRipple={true} className={parseInt(process.env.REACT_APP_TID_PSL) !== t.tid && classes.btnpsl} onClick={()=>handleClick(t.tid, t, v.name)} key={i} minWidth="100px" borderRadius="6px" display="flex" flexDirection="column" alignItems="center" gap="4px" py="8px" bgcolor="#F0F5F9"
                                border={parseInt(process.env.REACT_APP_TID_PSL) === t.tid && ("2px solid #3D77E9")}
                                >
