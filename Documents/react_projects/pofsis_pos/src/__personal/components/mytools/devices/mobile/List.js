                ...
                (v.tools.length > 0 || (v.tools !== undefined && v.tools !== null )) && (
                    <Box key={k} display="flex" flexDirection="column" gap="12px">
                        ...
                        <Box display="grid" gridTemplateColumns="repeat(auto-fill,minmax(100px,1fr))" gap="12px">
                            {v.tools.map((t,i) => (
                                <Box component={Button} disableFocusRipple={true} className={parseInt(process.env.REACT_APP_TID_PSL) !== t.tid && classes.btnpsl} onClick={()=>handleClick(t.tid, t, v.name)} key={i} minWidth="100px" borderRadius="6px" display="flex" flexDirection="column" alignItems="center" gap="12px" py="8px" bgcolor="#F0F5F9"
