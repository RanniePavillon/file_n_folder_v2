                        ...
                        <>
                            ...
                            <Box display="flex" alignItems="center" gap="24px">
                                ...
                                <Box minWidth='128px' display="flex" alignItems="center" gap="24px">
                                    <ToolTip bgcolor='primary.main' title={'Chat'} sx={{margin: '0 14px 0 14px'}}>
                                        ...
                                        navigation_state.tabs.data.top.map((v,k) => {
                                            return (
                                                <BizToolTip key={k} arrow title="Take a look at the tools you have." open={v.name === 'MyTools' && myToolPzl.data && !myTool.data} placement='bottom-start' sx={{fontSize:'12px', lineHeight:'20px'}}>
                                                    <Box display="flex" alignItems="center" height="100%" className='c-pointer'>
                                                        <ToolTip title={v.name} onClick={(e)=>NavHandler(e, v.name)}>
                                                            <Badge color="error" badgeContent={0}>
                                                                <IconButton 
                                                                    disableFocusRipple={true} 
                                                                    className={`${(v.name === 'MyTools' && myToolPzl.data && !myTool.data) && classes.btnbizfocus } ${v.className}`}
                                                                    sx={{
                                                                        padding:'4px !important',
                                                                        bgcolor: tabActive === v.name || __LOCATION === v.ref || (v.name === 'MyTools' && myTool.data) || (v.name === 'Notifications' && notif.data) ? '#F0F5F9' : '#FFFFFF', 
                                                                        borderRadius:'8px',
