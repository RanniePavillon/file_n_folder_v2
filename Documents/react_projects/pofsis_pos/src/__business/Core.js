...
const __SESSION = JSON.parse(localStorage.getItem('your_biz_session'))
const tool_id =  process.env.REACT_APP_TID_BIZ;
...
export const Core = () => {
	...
	useEffect(() => {
		...
		const __init = async () => {
			...
			localStorage.setItem('your_biz_session', JSON.stringify({expiration: `${d.getMonth()+1}${d.getUTCDate()}${d.getUTCFullYear()}`, ainfo: ainfo}))
		...
		console.log(`Your Tool Business Version V1.1.1 is running`);
